...
{
    public partial class GdShoppingCartController : ShoppingCartController
    {
		#region Fields
        ...
        public GdShoppingCartController(IProductService productService, 
            ...
            IGroupDealService groupDealService) : base(productService, storeContext,
                workContext,
                shoppingCartService,
                pictureService,
                localizationService,
        ...
        //add groupdeal to cart using AJAX
        //currently we use this method on the groupdeal details pages
        [HttpPost]
        [ValidateInput(false)]
        public ActionResult AddGroupDealToCart_Details(int groupDealId, int shoppingCartTypeId, FormCollection form)
        {
            ...
            //we can add only simple products
            if (groupDeal.ProductType != ProductType.SimpleProduct)
            {
                return Json(new
                {
                    success = false,
            ...
            foreach (string formKey in form.AllKeys)
                if (formKey.Equals(string.Format("addtocart_{0}.UpdatedShoppingCartItemId", groupDealId), StringComparison.InvariantCultureIgnoreCase))
                {
            ...
            if (_shoppingCartSettings.AllowCartItemEditing && updatecartitemid > 0)
            {
                ...
                //is it this product?
                if (groupDeal.Id != updatecartitem.ProductId)
                {
                    return Json(new
                    {
                        success = false,
            ...
            if (groupDeal.CustomerEntersPrice)
            {
                foreach (string formKey in form.AllKeys)
                {
                    if (formKey.Equals(string.Format("addtocart_{0}.CustomerEnteredPrice", groupDealId), StringComparison.InvariantCultureIgnoreCase))
                    {
            ...
            int quantity = 1;
            foreach (string formKey in form.AllKeys)
                if (formKey.Equals(string.Format("addtocart_{0}.EnteredQuantity", groupDealId), StringComparison.InvariantCultureIgnoreCase))
                {
            ...
            string attributes = ParseProductAttributes(groupDeal, form);
            
            //rental attributes
            DateTime? rentalStartDate = null;
            DateTime? rentalEndDate = null;
            if (groupDeal.IsRental)
            {
                ParseRentalDates(groupDeal, form, out rentalStartDate, out rentalEndDate);
            }
            ...
            if (updatecartitem == null)
            {
                ...
                addToCartWarnings.AddRange(_shoppingCartService.AddToCart(_workContext.CurrentCustomer,
                    groupDeal, cartType, _storeContext.CurrentStore.Id,
            ...
            else
            {
                ...
                var otherCartItemWithSameParameters = _shoppingCartService.FindShoppingCartItemInTheCart(
                    cart, cartType, groupDeal, attributes, customerEnteredPriceConverted,
                    rentalStartDate, rentalEndDate);
            ...
            switch (cartType)
            {
                case ShoppingCartType.Wishlist:
                    {
                        //activity log
                        _customerActivityService.InsertActivity("PublicStore.AddToWishlist", _localizationService.GetResource("ActivityLog.PublicStore.AddToWishlist"), groupDeal.Name);
                ...
                default:
                    {
                        //activity log
                        _customerActivityService.InsertActivity("PublicStore.AddToShoppingCart", _localizationService.GetResource("ActivityLog.PublicStore.AddToShoppingCart"), groupDeal.Name);
